...
from c_flat_optims.c_flat import C_Flat
...
def train(dataloader,
    ...
    running_time = 0
    iterations = 0
    optimizer = C_Flat(params=model.parameters(), 
                       base_optimizer=optimizer,
                       model=model,
                       cflat=True)
    
    # Pay attention if the scope needs reworking since variables are used without passing them as arguments
    def create_loss_fn(inputs, targets):
        def loss_fn_closure():
            new_logits, new_feat_map = model(inputs)
            cil_logits = new_logits[:, -cil_nr_of_classes]
            cil_labels = targets[:, -cil_nr_of_classes]
    ...
    for batch, (mel, label, fname) in enumerate(dataloader):
        ...
        loss_fn_closure = create_loss_fn(mel, label)
        optimizer.set_closure(loss_fn=loss_fn_closure)
        ...
        if use_cosine_kd:
            ...
            #sum_feat_loss += feat_loss.item()
            loss += feat_loss"""
        bce_loss, kld_loss, cos_loss = loss_list[0], loss_list[1], loss_list[2]
        total_loss = sum(loss_list)
        epoch_BCE_loss += bce_loss
        epoch_KLD_loss += kld_loss
        epoch_COS_loss += cos_loss
        ...
        if batch % log_interval == 0:
            ...
            training_loss, current = total_loss.item(), (batch + 1) * len(mel)
            epoch_training_loss += total_loss.item()
    ...
    print(f"Epoch COS loss: {epoch_COS_loss}")
